...
def post_user(user_data: dict, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Ошибка сервера при создании пользователя: {str(e)}"
        )
...
def update_user(user_id: int, user_data: dict, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка сервера при обновлении пользователя: {str(e)}"
        )
...
def delete_user(user_id: int, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка сервера при удалении пользователя: {str(e)}"
        )
...
def create_task(task_data: dict, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка сервера при создании задачи: {str(e)}"
        )
...
def update_task(task_id: int, task_data: dict, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка сервера при обновлении задачи: {str(e)}"
        )
...
def delete_task(task_id: int, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка сервера при удалении задачи: {str(e)}"
        )